			...
			'callback': function () {
				...
				$('.manageLayer a[class="ban_confirm"]').on('click', function (event) {
					...
					TOOLS.dpost(url, post,
						function (re) {
							$(obj).hide();
							$('.topicadminMsg[tid=' + topicAdmin.opts.tid + '][pid=' + topicAdmin.opts.pid + ']').html(!topicAdmin.opts.ban ? 'ÒÑÆÁ±Î' : '');
				...
				$('.manageLayer a[class="del_confirm"]').on('click', function (event) {
					...
					TOOLS.dpost(url, post,
						function (re) {
							$(obj).hide();
							if (topicAdmin.opts.isfirst) {
								$('.topicadminMsg[tid=' + topicAdmin.opts.tid + ']').html('ÒÑÉ¾³ý');
							} else {
								$('.topicadminMsg[tid=' + topicAdmin.opts.tid + '][pid=' + topicAdmin.opts.pid + ']').html('ÒÑÉ¾³ý');
							}
							TOOLS.showTips(re.Message.messagestr, true);
							TOOLS.setcookie('refreshindex', '1', 86400);
						},
