...
$limit = 50;
$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('a_'.$pluginName.'_order')." ");
...
$sql = "SELECT * FROM ".DB::table('a_'.$pluginName.'_order')." ORDER BY creation_time DESC LIMIT ".$start_limit." ,".$limit;
$query = DB::query($sql);
showsubtitle(array('',lang('plugin/'.$pluginName, 'Order number'),lang('plugin/'.$pluginName, 'lang65'),lang('plugin/'.$pluginName, 'Commit time'),lang('plugin/'.$pluginName, 'state'),lang('plugin/'.$pluginName, 'Over Time'),lang('plugin/'.$pluginName, 'user name'),lang('plugin/'.$pluginName, 'points'),lang('plugin/'.$pluginName, 'Recharge amount'),lang('plugin/'.$pluginName, 'Payment of RMB'),lang('plugin/'.$pluginName, 'lang49'),lang('plugin/'.$pluginName, 'lang50')));
