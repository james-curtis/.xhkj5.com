...
if($act=='twmsg'){
	...
	if(!in_array($_G['groupid'],unserialize($config['url_groups'])) && $url_groups[0]){
		$message=preg_replace("/\[url\=(.+?)\](.+?)\[\/url\]/is","<a href='$1'>$2</a>",$_GET['content']);

		$message = preg_replace("/<a[^>]*href=[^>]*>|<\/[^a]*a[^>]*>/i","",$message);
		$message=htmlspecialchars_decode(addslashes(html2bbcode($message)));
	}
	//判断内容是否存在
	if(!$message){
		if($_G['mobile']){
	...
	if($coin){
		...
		if($coin>$config['zgxsz']){
			if($_G['mobile']){
				echo "<script>parent.tips('".lang('plugin/ljww360','biggest_reward').$creditname.$config['zgxsz']."','');</script>";
				exit;
			}else{
				showerror(lang('plugin/ljww360','biggest_reward').$creditname.$config['zgxsz']);
			}
		}
		if($coin<0){
			if($_G['mobile']){
