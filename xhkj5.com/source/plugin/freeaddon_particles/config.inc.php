...
/**
 *序列号：2018031021tY2w2Y8hyC
 *应用版本：76704
 *发布时间：1507021201
 *站点URL：http://xhkj5.com/	
 *客户端URL：http://www.xhkj5.com/	
 *站点ID：83F864B4-FD0D-3931-3216-C607D44770E0	
 *QQID：DF130AF0-F61B-ECC5-0100-6F7824A62A67
 */
$pluginvars = array();
...
if(!submitcheck('editsubmit')) {
	$operation = '';
	if($pluginvars) {
		$_statInfo = array();$_statInfo['pluginName'] = $plugin['identifier'];$_statInfo['pluginVersion'] = $plugin['version'];$_statInfo['bbsVersion'] = DISCUZ_VERSION;$_statInfo['bbsRelease'] = DISCUZ_RELEASE;$_statInfo['timestamp'] = TIMESTAMP;$_statInfo['bbsUrl'] = $_G['siteurl'];$_statInfo['SiteUrl'] = 'http://xhkj5.com/';$_statInfo['ClientUrl'] = 'http://www.xhkj5.com/';$_statInfo['SiteID'] = '83F864B4-FD0D-3931-3216-C607D44770E0';$_statInfo['bbsAdminEMail'] = $_G['setting']['adminemail'];$_statInfo['genuine'] = splugin_genuine($plugin['identifier']);
		...
		foreach($pluginvars as $var) {
			...
			if($var['type'] == 'number') {
				...
				$var['type'] = 'calendar';//站点ID：83F864B4-FD0D-3931-3216-C607D44770E0	
				...
				$var['variable'] = $var['value'] = '';
			} elseif(substr($var['type'], 0, 5) == 'group') {//发布时间：1507021201
				if($var['type'] == 'groups') {
					$var['description'] = ($var['description'] ? (isset($lang[$var['description']]) ? $lang[$var['description']] : $var['description'])."\n" : '').$lang['plugins_edit_vars_multiselect_comment']."\n".$var['comment'];
					$var['value'] = dunserialize($var['value']);
				...
				$var['variable'] = $var['value'] = '';//客户端URL：http://www.xhkj5.com/
			} elseif($var['type'] == 'extcredit') {//应用版本：76704
				$var['type'] = '<select name="'.$var['variable'].'"><option value="">'.cplang('plugins_empty').'</option>';
				foreach($_G['setting']['extcredits'] as $id => $credit) {
					$var['type'] .= '<option value="'.$id.'"'.($var['value'] == $id ? ' selected' : '').'>'.$credit['title'].'</option>';
				}
				$var['type'] .= '</select>';
				$var['variable'] = $var['value'] = '';//站点URL：http://xhkj5.com/
			}
