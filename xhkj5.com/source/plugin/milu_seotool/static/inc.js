...
function st_format_url(url){
	...
	url = url.replace(reg3,'[YH123');
	url = url.replace(reg4,'K_L123');
	url = url.replace(reg5,'K_R123');
	url = url.replace(reg6,'XH123');
	url = url.replace(reg7,'DY123');
	url = url.replace(reg8,'XG123');
...
function _tips(key){
	$j.post(PLUGIN_URL+'member&myac=seotool_ajax_func&inajax=1&inajax=1&af=tips_no&key='+key, null,function (msg) {})
}
...
function plugin_tips(key){
	$j.post(PLUGIN_URL+'flink&myac=seotool_ajax_func&inajax=1&af=tips_no&key='+key, null,function (msg) {})
}
