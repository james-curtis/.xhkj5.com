...
sload('F:copyright');
if($_G['adminid'] < 1 ) exit('Access Denied:sb360');
$ac = $_GET['ac'];
if(!empty($ac) && function_exists($ac)) {
	if(!in_array($ac, array('plugin_check', 'plugin_download', 'plugin_install'))) exit('Access Denied:sb360');
	$info = $ac();
	return;
}	
...
function system_count(){
	...
	$arr['spider']['show'] = $spider_count.'<a style="padding-left:15px;" href="?'.PLUGIN_GO.'spider&myac=spider_data_clear">'.stlang('spider_data_clear').'</a>';
