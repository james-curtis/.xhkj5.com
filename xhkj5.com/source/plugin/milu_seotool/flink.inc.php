...
function flink_set(){
	...
	if($_GET['editsubmit']){
		$set = $_POST['set'];
		$set['link_type'] = $_POST['link_type'];
		$set['site_catid'] = $_POST['site_catid'];
		$set['limit_site_catid'] = st_serialize($_POST['limit_site_catid']);
		$set['limit_baidusnap'] = $_POST['limit_baidusnap'];
		$set['limit_pr'] = $_POST['limit_pr'];
...
function flink_onlie(){
	...
	$style = $_POST['more_flag'] == 1 ? '' : 'display:none';
	$search_show_more .=  '<div id="tipslis_1" style="'.$style.'">'.stlang('limit_baidu').' '.seoOutput::input(
		array(
		'name' => 'baidu',
		'length' => 3,
		'info' => $search_info,
	...
	$search_show_more .=  ' '.stlang('limit_google').' '.seoOutput::input(
		array(
		'name' => 'google',
		'length' => 3,
		'info' => $search_info,
	...
	$search_show_more .=  ' '.stlang('baidusnap').' '.seoOutput::select(
	...
	$search_show .=  ' '.stlang('order').' '.seoOutput::select(
		array('option_arr' => array('0' => stlang('default'), 'baidu' => stlang('baidu_count'), 'google' => stlang('google_count'), 'pr' => 'PR', 'baidusnap' => stlang('baidusnap'), 'modify_dateline' => stlang('modify_dateline'), 'score' => stlang('scoree_total')),
		'name' => 'order_by',
		'info' => $search_info,
		'int_val' => 10,
		'flag' => 2,
	...
	$info['search_show_more'] = $search_show_more;
	$info['flink_set'] = $flink_set;
	$info['more_flag'] = $_POST['more_flag'];
	return $info;
}
...
function flink_online_list($args){
	...
	$info['flink_success'] = $data['flink_success'];
