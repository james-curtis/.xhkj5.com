...
0
|| checktplrefresh('./template/xhkj5.com_iuni/touch/member/login.htm', './template/default/touch/common/seccheck.htm', 1524237830, '13', './data/template/15_13_touch_member_login.tpl.php', './template/xhkj5.com_iuni', 'touch/member/login')
...
<ul>
<li><input type="text" value="" tabindex="1" class="inp" autocomplete="off" value="" name="username" placeholder="请输入用户名" fwin="login"></li>
<li><input type="password" tabindex="2" class="inp"  value="" name="password" placeholder="密码" fwin="login"></li>
<li class="questionli">
...
<li class="bl_none answerli" style="display:none;"><input type="text" name="answer" id="answer_<?php echo $loginhash;?>" class="inp" placeholder="答案"></li>
<?php if($seccodecheck) { ?>
...
<?php } ?>
<li><button tabindex="3" value="true" name="submit" type="submit" class="formdialog btn1">登录</button></li>
<?php if($_G['setting']['regstatus']) { ?>
<li>
<a class="btn2" href="member.php?mod=<?php echo $_G['setting']['regname'];?>">还没有注册？</a>
</li>
<?php } if($_G['setting']['connect']['allow'] && !$_G['setting']['bbclosed']) { ?>
<li class="btn_qqlogin"><a href="<?php echo $_G['connect']['login_url'];?>&statfrom=login_simple">使用QQ帐号登录</a></li>
<?php } ?>
<li><?php if(!empty($_G['setting']['pluginhooks']['logging_bottom_mobile'])) echo $_G['setting']['pluginhooks']['logging_bottom_mobile'];?></li>
</ul>
</div>
</form>
