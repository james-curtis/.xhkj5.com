...
<a href="<?php if($_GET['action'] == 'newthread') { ?>forum.php?mod=forumdisplay&fid=<?php echo $_G['fid'];?>&page=<?php echo $_GET['page'];?><?php } else { ?>forum.php?mod=redirect&goto=findpost&ptid=<?php echo $_G['tid'];?>&pid=<?php echo $pid;?><?php } ?>" class="goBack fl">返回</a>
<h1><?php if($_GET['action'] == 'edit') { ?>编辑<?php } else { ?>发帖<?php } ?></h1>
</header>
<!-- header end -->
...
<input type="text" tabindex="1" class="inp" id="needsubject" size="30" autocomplete="off" value="<?php echo $postinfo['subject'];?>" name="subject" placeholder="标题" fwin="login">
<?php } else { ?>
...
<li>
<input type="checkbox" name="delete" id="delete" class="inp" value="1" title="删除本帖<?php if($thread['special'] == 3) { ?>，返还悬赏费用，不退还手续费<?php } ?>"> 删?
</li>
<?php } ?>
<li class="cfix">
<textarea class="textarea" id="needmessage" tabindex="3" autocomplete="off" id="<?php echo $editorid;?>_textarea" name="<?php echo $editor['textarea'];?>" cols="80" rows="2" placeholder="内容" fwin="reply"><?php echo $postinfo['message'];?></textarea>
</li>
...
<?php if($_GET['action'] == 'newthread') { ?>发表<?php } elseif($_GET['action'] == 'reply') { ?>回复<?php } elseif($_GET['action'] == 'edit') { ?>保存<?php } ?>
</button>
</div>
<!-- main postbox end -->
</form>
